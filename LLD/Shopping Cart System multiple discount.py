...
# - Applying multiple discounts to each cart item (e.g., 10 off plus 5% off on the same item).
# - Calculating the final payable amount after applying all discounts sequentially on each item.
...
if __name__ == "__main__":
    ...
    # Apply multiple discounts to items
    cart.items[0].add_discount(FixedAmountDiscount(200))      # Laptop: -$200
    cart.items[0].add_discount(PercentageDiscount(10))        # Laptop: -10% after fixed discount
    cart.items[1].add_discount(PercentageDiscount(10))        # Shoes: -10%
    cart.items[1].add_discount(FixedAmountDiscount(15))       # Shoes: -$15 after percent discount
    cart.items[2].add_discount(FixedAmountDiscount(50))       # Headphones: -$50
    cart.items[3].add_discount(PercentageDiscount(20))        # Book: -20%
    cart.items[3].add_discount(PercentageDiscount(10))        # Book: additional -10%
    cart.items[4].add_discount(FixedAmountDiscount(10))       # Backpack: -$10
    cart.items[4].add_discount(FixedAmountDiscount(5))        # Backpack: additional -$5
